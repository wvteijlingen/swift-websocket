...
struct WebSocketTests {
    ...
    func connect() async throws {
        ...
        #expect(webSocket.state == .connected)
    ...
    func disconnect() async throws {
        ...
        try await webSocket.connect()
        try webSocket.disconnect(closeCode: .normalClosure)

        try await Task.sleep(for: .seconds(1))

        #expect(webSocket.state == .disconnected)
    ...
    func disconnectWithCloseCodeAndReason() async throws {
        ...
        try await webSocket.connect()
        try webSocket.disconnect(closeCode: .goingAway, reason: "See you later")
